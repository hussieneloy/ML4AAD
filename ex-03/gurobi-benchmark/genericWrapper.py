...
class AbstractWrapper(object):
    ...
    def print_result_string(self):
        ...
        if self.args and self.args.log:
            ...
            with open("target_algo_runs.csv", "a") as fp:
                fp.write("%s, %d, %s, %f, " %(self._instance, self._seed, self._ta_status, self._ta_runtime))
                config = []
                for name, value in self._config_dict.items():
                    config.append("%s=%s" %(name, value))
                fp.write(", ".join(config))
                if (len(self._ta_misc) > 0):
                    fp.write(", %s" % (self._ta_misc))
                fp.write("\n")
                fp.flush()
