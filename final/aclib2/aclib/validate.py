...
class AClibVal(object):
    ...
    def get_args(self):
        ...
        req_group.add_argument("-c", "--configurator", required=True,
                               choices=["SMAC2", "PARAMILS", "ROAR", "SMAC3", "IRACE2", 
                                        "GGA", "GGA++", "GGA_def", "GGA++_def", "TOOL_CL_EACH",
                                        "TOOL_EF_EACH", "TOOL_UBC_EACH", "TOOL_CL_LIST",
                                        "TOOL_EF_LIST", "TOOL_UBC_LIST"
        ...
        exec_group.add_argument(
            "--env", choices=["local", "meta", "nemo"], default="local", help="environment to execute runs")
        exec_group.add_argument(
        ...
        val_group.add_argument(
            "--confs", choices=["DEF", "INC", "DEF+INC", "TIME"], required=True, help="what to validate")
        val_group.add_argument(
    ...
    def check_exec_dir(self, scenario):
        '''
        ...
        if os.path.isdir(exec_dir):
            ...
            os.mkdir(exec_dir)
            # Now create symlinks
            os.symlink(os.path.join(self.aclib_root, "configurators"),
                       os.path.join(exec_dir, "configurators"))
            os.symlink(os.path.join(self.aclib_root, "scenarios"),
                       os.path.join(exec_dir, "scenarios"))
            os.symlink(os.path.join(self.aclib_root, "target_algorithms"),
                       os.path.join(exec_dir, "target_algorithms"))
            os.symlink(os.path.join(self.aclib_root, "instances"),
    ...
    def create_validation_calls(self, scenario: str,
        ...
        if self.use_local_scenario:
            try:  # TODO: remove this later
                self.logger.warn("Using local scenario.txt, which could result in noisy validation results using the DB workers because all runs are validated independently.")
            ...
            scenario_fn = glob.glob(
                    os.path.join(path, "scenarios/*/%s/scenario.txt" % (scenario)))[0]
